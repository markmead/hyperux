...
>
  ...
  >
    ...
    <div x-ref="infiniteScrollLoader" class="h-px" aria-hidden="true"></div>
    <div x-show="isLoading">Loading&hellip;</div>
    <div x-show="isDisabled">All items loaded.</div>

    <p
      class="sr-only"
      aria-live="polite"
      x-text="isLoading ? 'Loading more items' : isDisabled ? 'All items loaded' : ''"
    ></p>
  </div>
</div>
```
...
```html
<button type="button" x-on:click="$refs.scroller.__x.$data.enableScroll()">Load more</button>

<div x-ref="scroller" x-data="huxInfiniteScroll({ scrollerId: 'list', startsDisabled: true })">
  ...
</div>
```
...
```html
<div x-on:hux-infinite-scroll:feed:load-more.window="loadMore($event.detail)">...</div>
```
...
- Announce loading, completion, and failure with a single always-present visually hidden `aria-live="polite"` region rather than putting `aria-live` on `x-show`-toggled indicators, which only change visibility and may not announce reliably.
...
- When the observer root is the scroll container (`useContainerRoot`), give that container `tabindex="0"` and an accessible name so keyboard-only users can focus it and scroll with arrow keys — otherwise, with no focusable list items, they cannot reach the loader to trigger the next page.
...
- Surface fetch failures (for example, a `loadError` flag set in `markFailed()`) in both the live region and a visible message so users know a load failed.
