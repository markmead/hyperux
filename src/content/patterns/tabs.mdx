...
- When `id` is set, `selectTab()` dispatches `hux-tabs:{id}:change` on `window` with `{ tabId }` in `event.detail`.
- When `id` is set, the initial active tab is also dispatched via `hux-tabs:{id}:change` after all components in the current render have initialized.
- `huxTabPanel` sets `isActive` to `true` when it receives a `hux-tabs:{tabsId}:change` event whose `tabId` matches its own `tabId`.
...
- Events emitted when `id` is set:
  - `hux-tabs:{id}:change` — dispatched on `window` with `{ tabId: string }` detail on init and on every `selectTab()` call.
